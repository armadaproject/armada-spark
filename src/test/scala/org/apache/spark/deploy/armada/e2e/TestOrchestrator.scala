...
class TestOrchestrator(
    ...
    resultFuture
      .andThen {
        case scala.util.Failure(ex) =>
          ...
          logger.info(s"[FAILURE] Test failed with exception: ${ex.getMessage}")
  ...
  private def submitJob(
    ...
    logger.info(s"[SUBMIT]   Application JAR: $sparkExamplesJar")
  ...
  private def watchJob(
    ...
    val jobStatus =
      try {
        ...
        case _: TimeoutException =>
          logger.info(s"[TIMEOUT] Job watch timed out after ${jobWatchTimeout.toSeconds}s")
          JobSetStatus.Timeout
        case ex: Exception =>
          logger.info(s"[ERROR] Job watch failed: ${ex.getMessage}")
          JobSetStatus.Failed
      } finally {
        jobCompleted = true
      }
      ...
      case Some(thread) =>
        try {
          ...
          case _: InterruptedException =>
            logger.info("[WARNING] Assertion thread was interrupted while waiting for completion")
            false
        }
    ...
    if (finalStatus != JobSetStatus.Success || hasAssertionFailures) {
      logger.info("[DEBUG] Test or assertions failed, capturing debug information...")

      // Print which assertions failed for clarity
      if (hasAssertionFailures) {
        logger.info("[DEBUG] Failed assertions:")

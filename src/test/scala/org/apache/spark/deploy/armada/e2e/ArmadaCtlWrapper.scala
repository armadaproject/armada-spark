...
class ArmadaCtlWrapper(val armadaUrl: String = "localhost:30002") {
  ...
  /** Parse YAML output directly to Queue object.
    *
    * This approach directly parses YAML to the upstream api.submit.Queue class:
    *   1. Jackson ignores unknown properties (apiVersion, kind)
    *   2. Parses directly to Queue protobuf object
    *   3. No intermediary case classes needed!
    *
    * @param yamlOutput
  ...
  private[e2e] def parseQueueYaml(yamlOutput: String): Try[Queue] = {
    Try {
      // Direct YAML → Queue parsing! Jackson ignores apiVersion, kind automatically

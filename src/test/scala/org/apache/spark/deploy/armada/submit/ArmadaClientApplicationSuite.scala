...
class ArmadaClientApplicationSuite extends AnyFunSuite with BeforeAndAfter with Matchers {
  ...
  private def createJobItemTemplateFile(
      priority: Double,
      namespace: String,
      filename: String = "job-item-template.json"
  ): File = {
    val templateContent =
      s"""{
         |  "priority": $priority,
  ...
  test("validateArmadaJobConfig should use queue from template when CLI config not provided") {
    ...
    config.cliConfig.queue shouldBe None                    // CLI config not provided
    config.cliConfig.jobSetId shouldBe Some("test-job-set") // CLI config takes precedence
  ...
  test("validateArmadaJobConfig should use jobSetId from template when CLI config not provided") {
    ...
    config.cliConfig.queue shouldBe Some("test-queue") // CLI config takes precedence
    config.cliConfig.jobSetId shouldBe None            // CLI config not provided
    config.jobTemplate should not be empty
    config.jobTemplate.get.jobSetId shouldBe "template-job-set"
  }
  ...
  test("validateArmadaJobConfig should load all three template types simultaneously") {
    ...
    val driverTemplateFile =
      createJobItemTemplateFile(1.5, "all-driver-namespace", "driver-template.json")
    val executorTemplateFile =
      createJobItemTemplateFile(2.5, "all-executor-namespace", "executor-template.json")
    ...
    config.cliConfig.queue shouldBe Some("test-queue")      // CLI value, not template value
    config.cliConfig.jobSetId shouldBe Some("test-job-set") // CLI value, not template value
  }

  // Tests for mergeTemplateWithConfig method using real template files
  ignore("mergeTemplateWithConfig should use config values over template values") {
  ...
  ignore("mergeTemplateWithConfig should handle empty job request items") {
  ...
  ignore("mergeDriverTemplate should merge template with runtime configuration") {
  ...
  test("mergeExecutorTemplate should merge template with runtime configuration") {
    ...
    val cliConfig = armadaClientApp.CLIConfig(
      ...
      driverLimitCores = Some("1"),
      driverRequestCores = Some("1"),
      driverLimitMemory = Some("1Gi"),
      driverRequestMemory = Some("1Gi"),
      executorLimitCores = Some("1"),
      executorRequestCores = Some("1"),
      executorLimitMemory = Some("1Gi"),
      executorRequestMemory = Some("1Gi")
    )
  ...
  ignore("mergeDriverTemplate should handle template without podSpec") {
    val template = JobSubmitRequestItem(
      priority = 1.0,
      namespace = "template-namespace",

...
class SimplePodMonitor(namespace: String) {
  ...
  def captureDebugInfo(): Unit = {
    logger.info(s"[DEBUG] Capturing debug info for namespace $namespace")
    ...
    try {
      ...
      if (podsResult.exitCode == 0) {
        ...
        podNames.foreach { podName =>
          try {
            logger.info(s"[DEBUG] Capturing logs for pod $podName")
            ...
            case e: Exception =>
              logger.info(s"[DEBUG] Failed to capture info for pod $podName: ${e.getMessage}")
          }
        }
      }
      ...
      case e: Exception =>
        logger.info(s"[DEBUG] Failed to capture debug info: ${e.getMessage}")
    }
  }

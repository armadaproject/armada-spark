...
object KubectlUtils {
  ...
  /** Helper to build kubectl commands with common flags */
  def buildCommand(
      operation: String,
      namespace: Option[String] = None,
      additionalArgs: Seq[String] = Seq.empty
  ): Seq[String] = {
    val base          = Seq("kubectl") ++ operation.split(" ")
    val withNamespace = namespace.map(ns => base ++ Seq("-n", ns)).getOrElse(base)
    withNamespace ++ additionalArgs
  }
}

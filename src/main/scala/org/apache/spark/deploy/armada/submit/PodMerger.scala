...
private[submit] object PodMerger {
  ...
  def mergePods(base: Pod, overriding: Pod): Pod = {
    ...
    // Strategic merge for named arrays - prevents duplicates!
    ...
    // PodBuilder magic: preserves ALL fields from overriding, we only override what we merged

...
/** Converts between fabric8 PodSpec and protobuf PodSpec.
  *
  * Protobuf classes have different structures than K8s YAML. fabric8 parses K8s YAML correctly, so
  * we convert: YAML → fabric8 → protobuf → Armada gRPC.
  *
  * Some fields may be hardcoded to None/empty for compatibility across Spark versions.
  */
object PodSpecConverter {
  ...
  def fabric8ToProtobuf(fabric8Spec: model.PodSpec): generated.PodSpec = {
    ...
    generated.PodSpec(
      ...
      os = None,
      // overhead not converted - rarely used
      overhead = Map.empty,
      ...
      // readinessGates not converted - rarely used
      readinessGates = Seq.empty,
      // resourceClaims not converted - K8s 1.26+ feature
      ...
      // schedulingGates not converted - K8s 1.27+ feature
  ...
  private[submit] def convertAffinityFromFabric8(a: model.Affinity): Option[generated.Affinity] = {
    ...
    val podAffinity     = None // Pod affinity not supported in protobuf schema
    val podAntiAffinity = None // Pod anti-affinity not supported in protobuf schema

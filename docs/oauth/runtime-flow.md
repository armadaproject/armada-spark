...
> **Mental model:** oauth2-proxy is a stateless reverse proxy. The "session" lives entirely in an encrypted cookie on the user's browser. Spark UI never learns about OAuth; it sees plain HTTP from `localhost:4180`.
